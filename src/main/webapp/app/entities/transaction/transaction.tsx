  ...
  return (
    <div>
      <h2 id="transaction-heading" data-cy="TransactionHeading">
        <Translate contentKey="bootcamp2023App.transaction.home.title">Transactions</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="bootcamp2023App.transaction.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/transaction/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="bootcamp2023App.transaction.home.createLabel">Create new Transaction</Translate>
          </Link>
        </div>
      </h2>
        ...
        {transactionList && transactionList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="bootcamp2023App.transaction.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('day')}>
                  <Translate contentKey="bootcamp2023App.transaction.day">Day</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="bootcamp2023App.transaction.client">Client</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {transactionList.map((transaction, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{transaction.day ? <TextFormat type="date" value={transaction.day} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{transaction.client ? <Link to={`/client/${transaction.client.id}`}>{transaction.client.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="bootcamp2023App.transaction.home.notFound">No Transactions found</Translate>
            </div>
          )
        )}
      </div>
